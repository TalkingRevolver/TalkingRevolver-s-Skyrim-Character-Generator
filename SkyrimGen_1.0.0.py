...
class Character:
    ...
    def __init__(self):
        ...
        self.god = None # Starting Wintersun deity
        ...
        self.wintFlag = False # Wintersun - Faiths of Skyrim
    ...
    def startProgram(self):
        ...
        if self.session != True:
            ...
            # Wintersun
            print('Are you using Wintersun - Faiths of Skyrim? Y or N')
            choice = input()
            if choice.upper() == 'Y':
                self.wintFlag = True
    ...
    # setGod
    # Recieves self.race and self.align
    # Randomly gets a string from dListR that matches a string from
    #       dListA, sets to self.god
    def setGod(self, race, align):
        # Race deity list
        rList = listModule.dListR.get(race)
        # Alignment deity list
        aList = listModule.dListA.get(align)

        # Get random deity from rList
        num = random.randint(0, len(rList) - 1)
        d = rList[num]
        ...
        # If d not in aList, re-generate
        while d not in aList:
            num = random.randint(0, len(rList) - 1)
            d = rList[num]
            if d == 'None':
                break
    ...
    # reroll
    # Recieves rNum, rerolls an item according to rNum
    def reroll(self, num):
        match num:
            case 1:
                self.setRace()
            ...
            case 4:
                self.setAlign()
            case 5:
                self.setGod(self.race, self.align)
            case 6:
                self.traits = []
    ...
    # Cretes file if one does not exist; otherwise, data will be appended
    def writeFile(self):
        try:
            # Open file, or create one if no file exists
            file = open('CharacterInfo.txt', mode='a', encoding='utf-8')
            # Date and time of generation
            time = datetime.datetime.now()
            file.write('Info generated on '+time.strftime('%m-%d,%Y')+' at '+
                   time.strftime('%H:%M:%S'))
            # Write info
            file.write('\nRace: ' + self.race)
            file.write('\nGender: ' + self.gender)

            if self.looks[0] in listModule.scalesList:
                file.write('\nSkin Color: ' + self.looks[0])
            elif self.looks[0] in listModule.furList:
                file.write('\nFur Color: ' + self.looks[0])
            else:
                file.write('\nComplexion: ' + self.looks[0])

            if self.looks[1] != None:
                file.write('\nHair Color: ' + self.looks[1])
        
            file.write('\nEye Color: ' + self.looks[2])

            if self.align != None:
                file.write('\nAlignment: ' + self.align)

            if self.god != None:
                file.write('\nStarting Deity: ' + self.god)
            ...
            if self.wPref != None:
                file.write('\n\tWeapon Preference: ' + self.wPref)
            if self.ePref != None:
                file.write('\n\tElement Preference: ' + self.ePref)

            if self.start != None:
                file.write('\n\tStart: ' + self.start)

            if self.db != None:
                file.write('\n\tDragonborn Status: ' + self.db)

            file.write('\n------------------------------------------------'+
                       '---\n')
            print('\nInfo wtitten to CharacterInfo.txt\n')
        finally:
            # Make sure file is closed if exception occurs at runtime
            file.close()
...
def main():
    ...
    while choice != '2':
        ...
        # If self.wintFlag is True, Call setGod
        if char.wintFlag == True:
            char.setGod(char.race, char.align)
        ...
        while True:
            ...
            input()
            # Ask user to write info to file
            print('Write character info to text file? Y or N')
            choice = input()
            if choice.upper() == 'Y':
                char.writeFile()
